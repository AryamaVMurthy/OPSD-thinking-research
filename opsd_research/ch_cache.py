...
from .ch_dossier import render_dynamic_teacher_dossier
...
def auditor_prompt(
    *,
    ...
    """Request a compact natural-language audit with full privileged context."""
    if len(attempts) not in {2, 3}:
        raise ValueError("the auditor requires two or three blind attempts")
    attempts_text = "\n\n".join(
        f"Blind attempt {index + 1}: {attempt}"
