...
def test_self_information_mixture_preserves_base_cross_entropy():
    base = _logits([2.0, 0.5, -0.5, -1.0])
    probability = base.softmax(dim=-1)
    log_probability = base.log_softmax(dim=-1)
    entropy_direction = log_probability - (
...
def test_self_information_mixture_stays_inside_positivity_boundary():
    base = _logits([0.0, 0.0, 0.0])
    direction = _logits([-10.0, 5.0, 5.0])
    ...
    assert result.metrics["positivity_limited"].item() == 1.0
    assert 0.0 < result.metrics["effective_step_size"].item() < 0.1
    assert result.metrics["minimum_mixture_ratio"].item() > 0.0
