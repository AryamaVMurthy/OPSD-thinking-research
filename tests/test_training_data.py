...
class TrainingDataTests(unittest.TestCase):
    ...
    def test_problem_only_partition_never_requires_a_response(self):
        dataset = [
            {
                "question": f"question-{index}",
                "data_source": "amc_aime" if index % 2 else "aops_forum",
            }
            for index in range(100)
        ]

        train_one, heldout_one = problem_partition_indices(dataset, 0.2)
        train_two, heldout_two = problem_partition_indices(
            list(reversed(list(reversed(dataset)))), 0.2
        )
        ...
        self.assertEqual(set(train_one).intersection(heldout_one), set())
        self.assertEqual(
            set(train_one).union(heldout_one), set(range(len(dataset)))
        )
        manifest = problem_partition_manifest(dataset, 0.2)
        self.assertEqual(
            manifest["partition_protocol"],
            "problem-provenance-sha256-first64-threshold-v1",
        )
        self.assertNotIn("response", str(manifest).lower())
        self.assertEqual(
            manifest["heldout_diagnostic_examples"], len(heldout_one)
        )
