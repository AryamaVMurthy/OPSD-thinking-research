...
manifest="${project_source}/logs/posttrain-4b-${submission_id}.tsv"
...
for checkpoint in 50 100 150 200; do
  for benchmark in aime25 hmmt25; do
    ...
    run_name="opsd-qwen3-4b-step${checkpoint}-${benchmark}"
...
checkpoint=200
config="reproductions/01_qwen3_thinking_math/configs/qwen3-4b-aime26.yaml"
run_name="opsd-qwen3-4b-step200-aime26"
eval_job="$(submit_eval "${config}" math_eval "${run_name}" "${checkpoint}")"
printf '%s\t%s\t%s\t\t%s\t%s\t%s\t%s\n' \
  "${source_commit}" "${train_job_id}" "${eval_job}" "${run_name}" \
  "${checkpoint}" "${config}" "${adapter_root}/checkpoint-${checkpoint}" \
...
config="reproductions/02_qwen3_thinking_livecodebench/configs/qwen3-4b-lcb-v6.yaml"
run_name="opsd-qwen3-4b-step200-lcb-v6"
