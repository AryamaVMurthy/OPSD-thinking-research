...
if (( $# != 3 )); then
  echo "usage: archive_lcb_eval.sh RUN_NAME GENERATION_JOB_ID SCORE_JOB_ID" >&2
  exit 2
fi
...
generation_log_path="artifacts/logs/opsd-eval-${generation_job_id}.out"
score_log_path="artifacts/logs/opsd-lcb-score-${score_job_id}.out"
telemetry_path="artifacts/telemetry/${run_name}-${generation_job_id}.csv"
comparison_path="artifacts/comparisons/${run_name}-vs-untouched.json"
